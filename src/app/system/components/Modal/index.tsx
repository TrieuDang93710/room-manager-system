...
const ModalResponsive = ({ isOpen, setIsOpen }: ModalResponsiveProps) => {
    ...
    >
      <div className='fixed z-30 animate-fade w-full sm:w-1/3 ssm:w-3/4 flex flex-col items-start justify-start gap-4 bg-slate-50 dark:bg-slate-900 min-h-screen px-3 sm:px-6 py-6'>
        ...
        <div className='w-full flex flex-col justify-center items-center py-1 px-2'>
          <Image alt='logo' src={job_logo} width={80} height={80} />
          <Link className='text-[18px] font-bold text-gradient-to-bl text-blue-600' href={'/'}>
            Jobs 24/7
          </Link>
        </div>
        ...
        {user ? (
          <div className='flex flex-col gap-4 pt-5 justify-center'>
            <Button onClick={logout} className='bg-none' variant={'outline'} size={'sm'}>
              Sign out
            </Button>
          </div>
        ) : (
