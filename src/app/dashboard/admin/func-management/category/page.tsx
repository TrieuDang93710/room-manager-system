...
const CategoryManagerPage = () => {
  ...
  useEffect(() => {
    setCategorySort(categories.sort((a: any, b: any) => a.id - b.id));
  }, [categories]);
  ...
  const headers = ['#', 'Tiêu đề nhóm nghề', 'Ngày tạo', 'Xử lý'];

  const renderRow = (cate: any) => (
    <>
      <td className='truncate px-2'>{cate.id}</td>
      <td className='truncate px-2'>{cate.title}</td>
      <td className='truncate px-2'>{cate.createAt}</td>
      ...
      <div className='w-full py-2 px-2 flex-col justify-center md:gap-3 gap-y-2'>
        ...
        <div className='md:flex md:flex-row flex-col gap-5 items-end justify-between my-6 px-2'>
          <div className='lg:w-[50%] md:w-2/3 w-full truncate flex justify-start gap-2'>
            <ButtonCommon onClick={closeAddFieldHandler} icon={<PlusOutlined />} title='Thêm nhóm nghề' />
            <ButtonCommon onClick={() => alert('Click me')} icon={<PlusOutlined />} title='Excel' />
          </div>
          <SearchComponent />
        </div>

        <TableComponent headers={headers} data={categorySort} renderRow={renderRow} />
