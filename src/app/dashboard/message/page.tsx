...
const MessagePage = () => {
  ...
  return (
    <div className='relative w-full h-screen bg-[#f7f7f7] dark:bg-[#242424] flex flex-col items-end gap-2 snap-y pt-20 md:px-3'>
      <div className='w-full h-full py-3 px-3 flex flex-col justify-start items-end md:gap-3 gap-y-2'>
        <div className='w-full flex sm:flex-row flex-col sm:items-center sm:justify-start items-start justify-start bg-blue-100 border shadow-sm shadow-slate-500 rounded-sm px-2 py-1 gap-1'>
        ...
        <form className='relative w-[80%] flex flex-row items-center bg-blue-200 mt-2'>
          <input
            type='text'
            placeholder='Type ...'
            className='w-full h-full px-2 py-2 bg-blue-200 focus:bg-[#0000003e] dark:focus:bg-transparent text-slate-800 dark:text-slate-50 dark:placeholder:text-slate-50 text-[18px]'
          />
          <button
            type='submit'
            onClick={sendHandler}
