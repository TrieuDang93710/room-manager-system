    ...
    <div className='container flex flex-wrap justify-start mt-2 py-2'>
      {breadcrumbs.map((link, index) => (
        <div key={link.url} className='flex items-center'>
          {link.url === currentUrl ? (
            <span
              className={`text-base cursor-default font-medium mr-2 gap-3 flex dark:text-blue-600 dark:hover:text-slate-100 text-blue-600`}
            >
            ...
            <Link href={link.url} className='text-black hover:text-blue-600 text-base cursor-pointer dark:text-blue-600 dark:hover:text-slate-100'>
              {link.prefixIcon && <link.prefixIcon />}
              <span className='font-medium mx-2'>{link.label}</span>
            </Link>
          )}
          {index !== breadcrumbs.length - 1 && (
            <span className={`text-s px-2 dark:text-gray-400 text-blue-600`}>
              <DoubleRightOutlined />
            </span>
          )}
        </div>
